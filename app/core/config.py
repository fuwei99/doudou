...
class Settings(BaseSettings):
    ...
    # Chrome 用户数据目录，用于持久化浏览器实例（Cookie、缓存、登录态等）
    # 默认保存在项目根目录下的 chrome_data 文件夹
    CHROME_USER_DATA_DIR: str = "chrome_data"
    # CDP 调试端口
    ...
    def validate_settings(self) -> "Settings":
        ...
        if cookies_env:
            self.DOUBAO_COOKIES.extend(
                [c.strip() for c in cookies_env.split("|") if c.strip()]
            )
